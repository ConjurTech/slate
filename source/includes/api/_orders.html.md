...
When an order created, the order matching engine will match the order against existing offers.
  If no offer is filled by the order, the matching engine will generate a `make` transaction within the order.
...
When an order created, the order matching engine will match the order against existing offers.
  If the order fills any offer, the matching engine will generate a `fill` transaction within the order.
  This transaction must be signed when using the [broadcast order endpoint](#execute-order).
