...
Do take note that orders from the same wallet address will not [fill](#the-fill-object) each another. 
  When two orders of the same wallet address cross each other, [Makes](#the-make-object) will be generated instead.
...
contract_hash              | Switcheo Exchange [contract hash](#contract-hash) that the order is on.
...
fills                      | Generated by the matching engine if the order fills other offer(s). Refer to the [fills](#the-fill-object) section for more details.
makes                      | Generated by the matching engine if any part of the order does not have the requirements to fill any existent offer. Refer to the [makes](#the-make-object) section for more details.
...
```
When an order created, the order matching engine will match the order against existing offers.
  If no offer is filled by the order, the matching engine will generate a `make` transaction.
  This transaction must be signed when using the [broadcast order endpoint](#execute-order).
...
available_amount           | Remaining [amount](#amounts) of the offered tokens in the make that has not been taken by other orders.
offer_asset_id             | [Asset ID](#supported-assets) of the token that the make is offering.
offer_amount               | [Amount](#amounts) of tokens that the make wants.
...
txn                        | The transaction or message representing this make, which must be signed during the broadcast step.
cancel_txn                 | If this make is cancelled, this parameter would be the transaction or message that represents the cancellation.
price                      | Buy or sell price of order to 8 decimal places precision.
status                     | Status of the make.
created_at                 | Time when the make is created.
transaction_hash           | Transaction hash of the transaction or message representing this make.
trades                     | The [trades](#trades) from other offers that are filling into this make.
...
When an order created, the order matching engine will match the order against existing offers.
  If the order fills any offer, the matching engine will generate a `fill` transaction.
  This transaction must be signed when using the [broadcast order endpoint](#execute-order).
...
fee_asset_id               | [Asset id](#supported-assets) of the token is used for fees.
fee_amount                 | [Amount](#amounts) of fees paid for the fill.
price                      | Buy or sell price of order to 8 decimal places precision.
txn                        | The transaction or message representing this fill, which must be signed during the broadcast step.
status                     | Status of the fill.
created_at                 | Time when the fill is created.
transaction_hash           | Transaction hash of the transaction or message representing this fill.
